...
def main():
    ...
    if verbose:
        ...
        image_dir = os.listdir('2024-02-27\cam1')
        out_dir = "output_stitch"

        dirs = ["2024-02-27\cam1", "2024-02-27\cam0"]

        for_loop_timer = Timer("For Loop Time")

        for img in image_dir:
            if counter > 2:
                break

            images = [os.path.join(dirs[0], img), os.path.join(dirs[1], img)]
            print("Starting to stitch")
            pano_timer = Timer("main.stitcher.stitch()")
            panorama = stitcher.stitch(images, feature_masks)
            pano_timer.stop()
            output = os.path.join(out_dir, img)
            cv.imwrite(output, panorama)
            counter += 1
